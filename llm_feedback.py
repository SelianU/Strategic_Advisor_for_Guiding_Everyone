...
- 집의 연결, 공격과 수비 균형, 핵심 자리 선점, 이후 수순의 단순화 여부를 중심으로 설명하세요.
- 숫자를 그대로 반복하기보다, 그 숫자가 의미하는 바를 풀어서 설명하세요.
- 영어 표현은 쓰지 마세요.
- 말투는 "했습니다", "좋았습니다", "유리했습니다"처럼 단정한 존댓말을 사용하세요.
- 인삿말, 이모티콘, 과한 감탄사는 넣지 마세요.
...
def build_gomoku_fallback_feedback(summary: dict[str, Any]) -> str:
    ...
    text = (
        ...
        f"실제 착수는 상대를 크게 압박하지 못한 반면, 권장 착수는 더 높은 가치로 평가되어 이후 전개에서 주도권을 잡기 쉬운 선택이었습니다.\n\n"
        f"비교 보드를 보면 인간 쪽은 이후 수순이 {human_seq}로 이어졌고, AI 권장 수를 따른 쪽은 {agent_seq}처럼 더 빠르게 핵심 자리를 선점했습니다. "
        f"이번 장면에서 중요한 점은 한 수의 위치 차이가 이후 수순 전체의 효율을 바꾼다는 점입니다."
    )
    return format_feedback_text(text)
