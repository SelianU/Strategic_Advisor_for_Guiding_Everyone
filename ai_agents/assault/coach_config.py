...
def build_user_prompt(summary: dict[str, Any]) -> str:
    ...
    image_note = (
        ...
        "적기 위치·고도·이동 방향, 에너지바 상태, 플레이어 포대 위치의 변화를 직접 확인해 상황을 묘사하세요."
        if has_image else ""
    )
...
- 첫 번째 문단: 플레이어님과 에이전트의 행동을 대비하세요. "에이전트 행동 해석"이 제공된 경우 이를 근거로, 에이전트가 어느 방향의 적기를 노렸고 왜 그 행동을 선택했는지 구체적으로 설명하세요. "이후 경로 활용 지침"에 따라 이후 전개도 포함하세요. 첨부 이미지 3장이 있다면 [직전]→[결정순간]→[직후] 흐름에서 보이는 적기 배치·에너지바 상태 변화를 배경으로 자연스럽게 녹이세요.
