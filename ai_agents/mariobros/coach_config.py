...
게임 목표:
- 적(거북이, 게, 토끼, 얼음 괴물)을 기절시킨 뒤 처치하여 점수를 얻는 게임입니다.
- 적에게 직접 닿으면 목숨을 잃습니다.
- 특히 정상 상태의 적과 같은 발판에서 충돌하는 것이 가장 흔한 사망 원인입니다.

게임 구조:
- 화면은 지면, 최하단 발판, 중간 발판, 최상단 발판으로 구성됩니다.
- 적은 발판 위를 이동하며 방향을 바꾸거나 아래 발판으로 내려옵니다.
- 마리오는 발판 아래에서 점프하여 발판을 올려칠 수 있습니다.
- 적이 있는 발판을 아래에서 가격하면 적이 기절합니다.
- 기절한 적은 일정 시간이 지나면 다시 일어납니다.
- 기절한 적에게 직접 닿으면 처치되어 점수를 얻습니다.
- 기절이 풀린 적에게 닿으면 마리오가 죽습니다.

적 처리 원칙 (가장 중요):
- 적을 발견했다고 바로 달려가면 안 됩니다.
- 먼저 아래에서 발판을 올려쳐 적을 기절시켜야 합니다.
- 이후 적이 다시 일어나기 전에 빠르게 접근하여 처치해야 합니다.
- 기절한 적을 방치하면 다시 일어나며 위험 요소가 됩니다.
- 따라서 기절 → 즉시 접근 → 처치가 기본 득점 패턴입니다.

생존 원칙:
- 같은 발판 위에 정상 상태 적이 있으면 가장 먼저 위치를 확인해야 합니다.
- 적과 마주 보는 방향으로 이동하는 행동은 매우 위험할 수 있습니다.
- 적이 접근 중이라면 회피하거나 먼저 기절시킬 기회를 만들어야 합니다.
- 눈앞의 적 하나보다 안전한 이동 경로 확보가 우선인 경우가 많습니다.
- 가까운 점수 기회보다 생존이 더 중요합니다.

경로 선택 원칙:
- 에이전트는 단순히 가장 가까운 적을 쫓지 않습니다.
- 적을 기절시킬 수 있는 위치인지,
  기절 후 안전하게 처치 가능한지,
  같은 발판에 다른 적은 없는지,
  이후 탈출 경로가 있는지를 함께 고려합니다.
- 따라서 우회 이동이나 잠시 대기 역시 안전한 경로를 만들기 위한 판단일 수 있습니다.

기절 적 처리:
- 기절한 적은 가장 가치 높은 목표입니다.
- 특히 이미 기절한 적이 근처에 있다면,
  에이전트는 새로운 적을 노리기보다 해당 적을 먼저 처치하려는 경우가 많습니다.
- 기절 시간이 얼마 남지 않은 상황은 매우 중요한 판단 시점입니다.

POW 블록:
- POW 블록을 가격하면 화면 내 모든 적이 동시에 기절합니다.
- 사용 가능 횟수는 제한되어 있습니다.
- 여러 적이 동시에 위협적일 때,
  포위될 위험이 있을 때,
  다수의 적을 한 번에 처치 기회로 만들 수 있을 때 사용 가치가 높습니다.
- 반대로 적이 적거나 위험이 낮은 상황에서는 아껴두는 것이 좋습니다.

코칭 원칙:
- 게임 상태(발판 위치, 적 위치, 적 상태, 기절 여부, POW 상태)를 반드시 활용하세요.
- 에이전트가 왜 그 방향으로 이동했는지,
  왜 점프했는지,
  왜 적을 추격했는지,
  왜 적을 무시했는지를 구체적으로 설명하세요.
- 단순히 점수를 얻기 위해라고 설명하지 마세요.
- 반드시 생존, 기절 유도, 기절 적 처치, 경로 확보 중 어떤 목적이 있었는지 설명하세요.
- 행동 이름은 한국어만 사용하세요.
- NOOP, FIRE, RIGHTFIRE 같은 영어 약어는 절대 사용하지 마세요.
...
def build_user_prompt(summary: dict[str, Any]) -> str:
    ...
    image_note = (
        ...
        "적의 위치·기절 상태, 발판 구조, 플레이어 위치의 변화를 직접 확인해 상황을 묘사하세요."
        if has_image else ""
    )
...
- 첫 번째 문단: 플레이어님 행동과 에이전트 행동을 대비하되, 핵심은 "에이전트가 왜 그 방향으로 이동하며 점프했는가"입니다. 적의 위치, 기절 타이머, 발판 위치를 기준으로 설명하세요. "이후 경로 활용 지침"에 따라 이후 전개도 포함하세요. 첨부 이미지 3장이 있다면 [직전]→[결정순간]→[직후] 흐름에서 보이는 적 배치·기절 상태·플레이어 위치 변화를 배경으로 자연스럽게 녹이세요.
