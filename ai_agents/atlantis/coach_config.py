...
def build_user_prompt(summary: dict[str, Any]) -> str:
    ...
    image_note = (
        ...
        "적의 x축 위치·고도·종류(고곤/밴디트/타나토이드), 활성 포대 발사각, 지상 시설 피해 상태의 변화를 직접 확인해 상황을 묘사하세요."
        if has_image else ""
    )
...
- 첫 번째 문단: 플레이어님 행동과 에이전트 행동을 명확히 대비하고, "이후 경로 활용 지침"에 따라 이후 전개를 설명하세요. "주목할 상황"이 제공된 경우, 나열하지 말고 배경으로 자연스럽게 녹이세요. 첨부 이미지 3장이 있다면 [직전]→[결정순간]→[직후] 흐름에서 보이는 적 위치·고도·포대 발사각 변화를 배경으로 자연스럽게 녹이세요.
