"""
gradcam_value.py  —  D3QN Value Stream Grad-CAM 시각화

Grad-CAM을 Value stream에 적용:
  - 상태 가치 V(s)에 대해
  - 마지막 Conv layer(features[-1]) feature map의 기여도를 계산
  - 원본 게임 화면에 heatmap으로 오버레이

  Advantage stream과의 차이:
  - Advantage: "어떤 행동이 유리한가?" → 적/총알 위치에 집중
  - Value:     "이 상황이 전반적으로 얼마나 좋은가?" → 생존 관련 구조 집중

사용법:
    python gradcam_value.py
    python gradcam_value.py --model ./checkpoints_v3/best_model.pth --frames 8 --out ./gradcam_value_out

출력:
    gradcam_value_out/gradcam_grid.png   ← 논문용 그리드 이미지
    gradcam_value_out/frame_XXX.png      ← 개별 프레임
"""
...
DEFAULT_OUT_DIR = "./gradcam_value_out"
...
# =============================================================================
# Grad-CAM (Value stream)
# =============================================================================
class ValueGradCAM:
    """
    Grad-CAM for the Value stream of DuelingDQN.

    타겟: V(s)  — 현재 상태의 가치 추정값
    ...
    Advantage와의 차이:
        Advantage → 행동 선택에 민감한 영역 (적, 총알 위치)
        Value     → 생존/점수에 전반적으로 중요한 영역 (화면 구조, 생존 공간)
    """
    ...
    def __call__(self, state_uint8: np.ndarray):
        ...
        # ── forward (value stream만 분리해서 grad 계산) ──────────────────
        ...
        # ── backward: V(s) 에 대해 (Value stream) ───────────────────────────
        target = value[0, 0]
        target.backward()
        ...
        return (heatmap,
                chosen_action,
                advantage[0].detach().cpu().numpy(),   # 행동별 advantage (표시용)
                q_values[0].detach().cpu().numpy(),
                value[0, 0].item())                    # V(s) 스칼라
...
def visualize_frame(raw_rgb, state_uint8, heatmap,
                    ...
                    frame_idx, score, out_path, v_scalar=0.0):
    """
    ...
    ax3.set_title("Grad-CAM\n(Value Stream)", color='white', fontsize=10, pad=4)
    ax3.axis('off')
    ...
    fig.suptitle(
        ...
        f"Action: {action_name}  |  V(s): {v_scalar:.2f}  |  max Q: {q_values[action]:.2f}",
...
def make_grid(frame_data, out_path):
    ...
    fig.suptitle("D3QN Grad-CAM  |  Value Stream  |  ALE/SpaceInvaders-v5",
...
def main():
    ...
    grad_cam = ValueGradCAM(net, device)
    ...
    while captured < args.frames:
        # Grad-CAM 계산
        heatmap, action, advantage, q_values, v_scalar = grad_cam(state)
